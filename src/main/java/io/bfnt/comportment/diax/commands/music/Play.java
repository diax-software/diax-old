...
public class Play extends DiaxCommand {
    ...
    public void execute(Message trigger) {
        if (trigger.getGuild().getMember(trigger.getAuthor()).getVoiceState().inVoiceChannel()) {
            MusicUtil.loadAndPlay(MusicUtil.getMusicManager(trigger.getTextChannel()), trigger.getGuild().getMember(trigger.getAuthor()).getVoiceState().getChannel(), trigger.getTextChannel(), trigger.getRawContent().split(" ")[1]);
        } else {
